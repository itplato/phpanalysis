...
class PhpAnalysis
{
    ...
    */
    ...
    public function StartAnalysis( $optimize = TRUE )
    {
        ...
        $s = 0;
        $ansiWordMatch = "[0-9a-z@#%\+\._-]";
        $notNumberMatch = "[a-z@#%_\+]";
        for($i=0; $i < $slen; $i++)
        {
    ...
    public function StartSeoAnalysis( $sortby="count", $optimize = TRUE )
    {
        ...
        if( $sortby=='rank' )
        {
            foreach($words as $w => $c)
            {
                ...
                if( !empty($i[0]) )  $words[$w] = sprintf("%0.3f", $c * 100 / $i[0]);
                else $words[$w] = $c / 10;
            }
            arsort($words);
        }
    ...
    private function _optimize_result( &$smarr, $spos )
    {
       ...
       for(; $i < $arlen; $i++)
       {
            ...
            else if( isset( $this->addonDic['n'][ $smarr[$i] ] ) )
            {
                ...
                {
                    $newarr[$j] = $cw.$nw;
                    //echo $this->_iconv(UCS2, 'utf-8', $newarr[$j])."<br />";
                    //尝试检测第三个词
            ...
            else if($this->unitWord)
            {
                ...
                */
                else if( strlen($cw)==2 && strlen($nw)==2 && 
                        ( isset($this->hotChars[$cw]) || isset($this->hotChars[$nw]) )
                )
                {
                    $newarr[$j] = $cw.$nw;
                    //尝试检测第三个词
            ...
            if( !$ischeck )
            {
                ...
                {
                    if( !isset($newarr[$j-1]) || strlen($newarr[$j-1]) > 2 )
                    {
                        $newarr[$j] = $cw.$nw;
                        if( !isset($this->newWords[$newarr[$j]]) )
                        {
     ...
     */
     public function GetFinallyResult($spword=' ', $word_meanings=FALSE)
     {
        ...
        foreach($this->finallyResult as $v)
        {
            ...
            if($word_meanings) {
                $rsstr .= $spword.$w.$this->GetWordProperty($v['w']);
            }
            else {
                $rsstr .= $spword.$w;
            }
