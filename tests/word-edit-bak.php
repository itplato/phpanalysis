...
$dicFile = $normalDicSource;
//$dicFile = '../dict/not-build/tmp/---nj_15.txt';
$maxpage = 188;
...
if( !empty($_POST) )
{
        ...
        if( $wl[0]=='#' )
        {
            ...
            if( $p == $id )
            {
                ...
                foreach($_POST['word'] as $k => $w)
                {
                    if( trim($w)=='' ) continue;
                    //echo "{$w}<br>\n";
                    $str .= "{$w},".trim($_POST['rank'][$k]).",".trim($_POST['type'][$k]).",".trim($_POST['trade'][$k]).",".trim($_POST['level'][$k]).",".trim($_POST['info'][$k]).",\n";
                }
            }
            else
            {
    ...
    unset( $old_dicts );
    $_POST = array();
    file_put_contents($dicFile, rtrim($str)) or die("文件无法写入！");
    if( $pagenext==1 ) $id++;
...
<div class="form-group">
    ...
    <a style="cursor:pointer" v-on:click="showBaidu(word.w)">{{key}}</a>
    </label>
    <div class="col-sm-2">
    ...
    methods: {
        showBaidu:function(keyword){
            acUnit.ShowMessageBox({
                ...
                //html:"<iframe name='f1' src='?keyword="+keyword+"' style='width:560px;height:420px'></iframe>",
            });
        },
    }
});
